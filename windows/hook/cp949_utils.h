...
#ifndef CP949_UTILS_H
#define CP949_UTILS_H

#include <stdint.h>

// CP949 to Unicode lookup table (2350 entries)
// Index = (lead - 0xB0) * 94 + (trail - 0xA1)
static const uint16_t cp949_to_unicode_table[2350] = {
...
/**
 * CP949 2바이트를 Unicode로 변환 (테이블 참조)
 */
static inline uint32_t cp949_to_unicode(uint8_t lead, uint8_t trail) {
    if (lead < 0xB0 || lead > 0xC8) return 0;
    if (trail < 0xA1 || trail > 0xFE) return 0;
    int index = (lead - 0xB0) * 94 + (trail - 0xA1);
    return cp949_to_unicode_table[index];
}
...
static inline uint16_t cp949_to_glyph_index(uint8_t lead, uint8_t trail) {
    if (lead < 0xB0 || lead > 0xC8) return (uint16_t)'?';
    if (trail < 0xA1 || trail > 0xFE) return (uint16_t)'?';
    return 256 + (lead - 0xB0) * 94 + (trail - 0xA1);
}

static inline int is_cp949_lead(uint8_t byte) {
    return (byte >= 0xB0 && byte <= 0xC8);
}

static inline int is_cp949_trail(uint8_t byte) {
